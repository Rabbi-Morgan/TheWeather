...
export const feelsCool = (value: number): string => {
  if (value<=7) return "Freezing cold"
  if(value>7 && value <=15) return "Feels Colder"
  if(value>15 && value <=30) return "Feels warm"
  return "Flaming hot"
}

export const readPressure = (value: number): string => {
  if(value <= 1000) return "In for a strorm"
  if(value>1000 && value<=1015) return "A pretty average pressure"

  return "Good weather will set in"
}